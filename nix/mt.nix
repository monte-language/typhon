...
stdenv.mkDerivation {
    ...
    installPhase = ''
      ...
      echo "${pypy}/bin/pypy $out/nix/mt-bake.py; ${nix}/bin/nix-build -E \"let pkgs = import <nixpkgs> {}; in pkgs.callPackage $out/nix/montePackage.nix { typhonVm = ${typhonVm}; mast = ${mast}; } ./mt-lock.json \"" > $out/bin/mt-bake
