...
def [=> makeBuilder, => expand] := import("lib/monte/monte_fast_expander")
def tests := [].diverge()
...
def expandit(code):
    def b := makeBuilder()
    def node := parseModule(makeMonteLexer(trim(code), "<test>"), b, throw)
    return expand(b, astBuilder, throw)

for item in fixedPointSpecimens:
    tests.push(fn assert {
        def node := parseModule(makeMonteLexer(trim(item), "<test>"),
                                astBuilder, throw)
        assert.equal(expandit(item).canonical(), node.canonical())
    })
...
for [specimen, result] in specimens:
    tests.push(fn assert {
        def resultNode := parseModule(
            ...
            astBuilder, throw)
        assert.equal(expandit(specimen).canonical(), resultNode.canonical())
    })
