...
from typhon.nodes import Expr, Sequence, evaluate, interactiveCompile
...
def obtainModule(libraryPaths, filePath, recorder):
    for libraryPath in libraryPaths:
        ...
        term = tryExtensions(path, recorder)
        if term is None:
            continue

        # Cache.
        moduleCache.cache[path] = term
        return term
    else:
