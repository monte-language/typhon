...
from typhon.rfinger import makeFingerTreeClass
...
# ConstLists backed by finger trees. Finger lists.

FingerList = makeFingerTreeClass(0, operator.add, lambda _: 1)

def unwrapFingerList(obj, ej=None):
    """
    Get a finger list from a ConstList or FlexList.
    """
    ...
    if isinstance(l, FlexList):
        ...
        fl = FingerList()
        for value in l.snapshot():
            fl = fl.pushRight(value)
    throwStr(ej, u"Not a list!")

def wrapFingerList(fl):
    return ConstList(fl)

def isFingerList(obj):
    from typhon.objects.refs import resolution
    obj = resolution(obj)
...
@autohelp
class ConstListIterator(Object):
    """
    An iterator on a list, producing its elements.
    """
    ...
    def next(self, ej):
        if self.fl.isEmpty():
            ...
            value, self.fl = self.fl.popLeft()
            index = self._index
            rv = [IntObject(index), value]
            self._index += 1
            return rv
...
class ConstList(Object):
    ...
    @method("FingerList", "FingerList")
    @profileTyphon("List.add/1")
    def add(self, fl):
        return self.fl.add(fl)

    @method("FingerList", "List")
    ...
    def join(self, pieces):
        ...
        for piece in pieces:
            ...
            fl = fl.pushRight(piece)
        return fl

    @method("Any")
    def diverge(self):
        return FlexList(self.fl.asList())
    ...
    def get(self, index):
        ...
        if index >= self.fl.measure:
            raise userError(u"get/1: Index %d is out of bounds" % index)
        pred = LTPred(index)
        _, right = self.fl.split(pred.run)
        value, _ = right.popLeft()
        return value
    ...
    def last(self):
        if self.fl.isEmpty():
            ...
            value, _ = self.fl.popRight()
            return value

    @method("FingerList", "Int")
    def multiply(self, count):
        """
        Create a new list by repeating this list's contents.
        """
    ...
    def reverse(self):
        ...
        while not old.isEmpty():
            value, old = old.viewLeft()
            new = new.pushLeft(value)
        return new

    @method("FingerList", "Int", "Any", _verb="with")
    ...
    @method("Any")
    def _makeIterator(self):
        return ConstListIterator(self.fl)

    @method("Map")
    def asMap(self):
        from typhon.objects.collections.maps import monteMap
        d = monteMap()
        for i, o in enumerate(self.fl.asList()):
            d[IntObject(i)] = o
        return d
    ...
    def asSet(self):
        ...
        d = monteSet()
        for o in self.fl.asList():
            d[o] = None
        return d

    @method("Int", "FingerList")
    ...
    def op__cmp(self, other):
        ...
        while not left.isEmpty():
            ...
            leftValue, left = left.popLeft()
            rightValue, right = right.popRight()

            try:
                result = unwrapInt(leftValue.call(u"op__cmp", [rightValue]))
            except UserException:
                result = -unwrapInt(rightValue.call(u"op__cmp", [leftValue]))
            if result < 0:
                return -1
            if result > 0:
                return 1
        # If left was shorter than right, then right will be non-empty at this
        # point. Do a final check.
        return 0 if right.isEmpty() else -1

    @method("Bool", "Any")
    @profileTyphon("List.contains/1")
    def contains(self, needle):
        from typhon.objects.equality import EQUAL, optSame
        for specimen in self.fl.asList():
            if optSame(needle, specimen) is EQUAL:
                return True
        return False
    ...
    def indexOf(self, needle):
        from typhon.objects.equality import EQUAL, optSame
        for index, specimen in enumerate(self.fl.asList()):
            if optSame(needle, specimen) is EQUAL:
                return index
        return -1

    @method.py("FingerList", "Any", _verb="with")
    @profileTyphon("List.with/1")
    def with_(self, obj):
        return self.fl.pushRight(obj)

    @method.py("FingerList", "Int", "Any")
    def put(self, index, value):
        if not 0 <= index <= self.fl.measure:
            raise userError(u"put/2: Index %d out of bounds for list of length %d" %
                            (index, self.fl.measure))
        pred = LTPred(index)
        left, right = self.fl.split(pred.run)
        if right.isEmpty():
            # Append to top.
            return left.pushRight(value)
        else:
            # Replace.
            _, right = right.popLeft()
            right = right.pushLeft(value)
            return left.add(right)

    @method.py("Int")
    @elidable
    def size(self):
        return self.fl.measure
    ...
    @method("FingerList", "Int", "Int", _verb="slice")
    def _slice(self, start, stop):
        if start < 0:
            raise userError(u"slice/1: Negative start")
        if stop < 0:
            raise userError(u"slice/2: Negative stop")
        # Two splits. We do the right-hand split first since that lets us
        # avoid slice arithmetic.
        pred = LTPred(stop)
        left, _ = self.fl.split(pred.run)
        pred = LTPred(start)
        _, rv = left.split(pred.run)
        return rv
    ...
    @method("Int", "FingerList")
    def startOf(self, needle, start=0):
        return self._startOf(needle, 0)

    @method.py("Int", "FingerList", "Int", _verb="startOf")
    def _startOf(self, needle, start):
        if start < 0:
            raise userError(u"startOf/2: Negative start %d not permitted" %
                    start)
        pred = LTPred(start)
        _, fl = self.fl.split(pred.run)

        # This is quadratic. It could be better.
        index = start
        needleList = needle.asList()
        from typhon.objects.equality import EQUAL, optSame
        while not fl.isEmpty():
            haystack = fl
            for value in needleList:
                if haystack.isEmpty():
                    break
                haystackValue, haystack = haystack.popLeft()
                if optSame(haystackValue, value) is not EQUAL:
                    break
            else:
                return index
            index += 1
