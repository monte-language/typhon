...
def deepFrozenSupersetOf(guard):
    ...
    from typhon.objects.guards import AnyOfGuard, FinalSlotGuard, SameGuard
    from typhon.prelude import getGlobal
    if guard is deepFrozenGuard:
        return wrapBool(True)
    if guard is deepFrozenStamp:
        return wrapBool(True)
    if guard is getGlobal(u"Bool"):
        return wrapBool(True)
    if guard is getGlobal(u"Char"):
        return wrapBool(True)
    if guard is getGlobal(u"Double"):
        return wrapBool(True)
    if guard is getGlobal(u"Int"):
        return wrapBool(True)
    if guard is getGlobal(u"Str"):
        return wrapBool(True)
    if guard is getGlobal(u"Void"):
        return wrapBool(True)
    ...
    for superGuardName in [u"List", u"NullOk", u"Set"]:
        superGuard = getGlobal(superGuardName)
        ej = Ejector()
        try:
    ...
    ej = Ejector()
    try:
        guardPair = getGlobal(u"Map").call(u"extractGuards", [guard, ej])
        ...
        pass
    if (getGlobal(u"SubrangeGuard")
            .call(u"get", [deepFrozenGuard])
            .call(u"passes", [guard])
            is wrapBool(True)):
        return wrapBool(True)
...
def auditDeepFrozen(audition):
    ...
    if not isinstance(audition, Audition):
        userError(u"not invoked with an Audition")
    ast = audition.ast
    if not isinstance(ast, Obj):
        userError(u"audition not created with an object expr")
    # n = ast._n
